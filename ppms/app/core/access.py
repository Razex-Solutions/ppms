...
def require_admin(current_user: User) -> None:
    if current_user.role.name not in {"Admin", "MasterAdmin"} and not is_platform_user(current_user):
...
def require_station_access(current_user: User, station_id: int, detail: str = "Not authorized for this station") -> None:
    if current_user.role.name in {"Admin", "MasterAdmin"} or is_platform_user(current_user):
        return
